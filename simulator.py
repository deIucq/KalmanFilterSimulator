...
input_value = [2]*int(timecount/dt)                               # システムへの入力u(t) 今回は等加速度運動とした
...
kalmangain = [np.array([[0.01, 0],[0, 0.02]])]*int(timecount/dt) #カルマンゲイン
...
for t in range(int(timecount/dt)):
    ...
    estimate_position[t] = (np.eye(2) - kalmangain[t])@odometory[t] + kalmangain[t]@observation[t]

fig = plt.figure()
ax_x = fig.add_subplot(2,1,1,xlabel='t',ylabel='x')
ax_v = fig.add_subplot(2,1,2,xlabel='t',ylabel='v')

#ax_x.plot(np.arange(0, timecount, dt), [i[0] for i in odometory], label='odometory')
#ax_v.plot(np.arange(0, timecount, dt), [i[1] for i in odometory], label='odometory')
